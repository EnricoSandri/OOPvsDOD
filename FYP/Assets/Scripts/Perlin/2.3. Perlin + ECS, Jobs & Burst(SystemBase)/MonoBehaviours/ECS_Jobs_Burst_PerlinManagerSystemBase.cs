...
public class ECS_Jobs_Burst_PerlinManagerSystemBase : MonoBehaviour
{
    //Instance of this class.
    public static ECS_Jobs_Burst_PerlinManagerSystemBase instance;
    ...
    private void Awake()
    {
        ...
        for (int z = -worldHalfSize; z <= worldHalfSize; z++)
        {
            for (int x = -worldHalfSize; x <= worldHalfSize; x++)
            {
                ...
                EntityManager.SetComponentData(entityInstance, new Job_Burst_CubeDataSystemBase { initialPosition = position });
            }
        }
    }
}
