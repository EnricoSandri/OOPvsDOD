...
public class ECS_PerlinManagerSystemBase : MonoBehaviour
{
    //Instance of this class.
    public static ECS_PerlinManagerSystemBase instance;
    ...
    private void Awake()
    {
        ...
        for (int z = -worldHalfSize; z <= worldHalfSize; z++)
        {
            for (int x = -worldHalfSize; x <= worldHalfSize; x++)
            {
                ...
                EntityManager.SetComponentData(entityInstance, new ECS_CubeDataSystemBase { initialPosition = position });
            }
        }
    }
}
