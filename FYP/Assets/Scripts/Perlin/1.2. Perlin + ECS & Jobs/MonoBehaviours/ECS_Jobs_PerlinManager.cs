...
public class ECS_Jobs_PerlinManager : MonoBehaviour
{
    //Instance of this class.
    public static ECS_Jobs_PerlinManager instance;
    ...
    private void Awake()
    {
        ...
        for (int z = -worldHalfSize; z <= worldHalfSize; z++)
        {
            for (int x = -worldHalfSize; x <= worldHalfSize; x++)
            {
                ...
                EntityManager.SetComponentData(entityInstance, new ECS_Jobs_CubeData { initialPosition = position });
            }
        }
    }
}
